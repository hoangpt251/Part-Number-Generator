...
TRANSDUCER_FILE = os.path.join(BASE_DIR, "core", "data", "transducer.xlsx")

# Đọc các sheet một lần khi import module
_sheets = {}
if os.path.exists(TRANSDUCER_FILE):
    with pd.ExcelFile(TRANSDUCER_FILE) as xls:
        try:
            ...
            print(f"Lỗi đọc file transducer.xlsx: {e}")
            _sheets = {}
else:
    print("Không tìm thấy file transducer.xlsx, chỉ sử dụng entry fields")
    _sheets = {}

class TransducerLogic:
    """
    ...
    def generate_description(self, class_code, display_values, code_values, group="Other"):
        if group == "ABM":
            ...
            return f"Chờ sang phiên bản cập nhật sau bạn nhé :D"
