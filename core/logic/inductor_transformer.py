...
INDUCTOR_TRANSFORMER_FILE = os.path.join(BASE_DIR, "core", "data", "inductor_transformer.xlsx")

# Đọc các sheet một lần khi import module
_sheets = {}
if os.path.exists(INDUCTOR_TRANSFORMER_FILE):
    with pd.ExcelFile(INDUCTOR_TRANSFORMER_FILE) as xls:
        try:
            ...
            print(f"Lỗi đọc file inductor_transformer.xlsx: {e}")
            _sheets = {}
else:
    print("Không tìm thấy file inductor_transformer.xlsx, chỉ sử dụng entry fields")
    _sheets = {}

class Inductor_TransformerLogic:
    """
    ...
    def generate_description(self, class_code, display_values, code_values, group="Other"):
        if group == "ABM":
            ...
            return f"Tính năng này sẽ được cập nhật thêm ở phiên bản sau, hihi"
