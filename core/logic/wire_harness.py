...
WIRE_HARNESS_FILE = os.path.join(BASE_DIR, "core", "data", "wire_harness.xlsx")

# Đọc các sheet một lần khi import module
_sheets = {}
if os.path.exists(WIRE_HARNESS_FILE):
    with pd.ExcelFile(WIRE_HARNESS_FILE) as xls:
        try:
            ...
            print(f"Lỗi đọc file wire_harness.xlsx: {e}")
            _sheets = {}
else:
    print("Không tìm thấy file wire_harness.xlsx, chỉ sử dụng entry fields")
    _sheets = {}

class Wire_HarnessLogic:
    """
    ...
    def generate_description(self, class_code, display_values, code_values, group="Other"):
        ...
        if group == "ABM":
            ...
            return f"Chờ phiên bản sau mình làm tiếp nhá, hihi"
