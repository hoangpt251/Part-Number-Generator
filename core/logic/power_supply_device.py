...
POWER_SUPPLY_DEVICE_FILE = os.path.join(BASE_DIR, "core", "data", "power_supply_device.xlsx")

# Đọc các sheet một lần khi import module
_sheets = {}
if os.path.exists(POWER_SUPPLY_DEVICE_FILE):
    with pd.ExcelFile(POWER_SUPPLY_DEVICE_FILE) as xls:
        try:
            ...
            print(f"Lỗi đọc file power_supply_device.xlsx: {e}")
            _sheets = {}
else:
    print("Không tìm thấy file power_supply_device.xlsx, chỉ sử dụng entry fields")
    _sheets = {}

class Power_Supply_DeviceLogic:
    """
    ...
    def generate_description(self, class_code, display_values, code_values, group="Other"):
        if group == "ABM":
            ...
            return f"Chờ sang phiên bản sau nhé, hihi"
