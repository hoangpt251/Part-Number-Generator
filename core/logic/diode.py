...
DIODE_FILE = os.path.join(BASE_DIR, "core", "data", "diode.xlsx")

# Đọc file nếu tồn tại, nếu không thì _sheets rỗng
_sheets = {}
if os.path.exists(DIODE_FILE):
    with pd.ExcelFile(DIODE_FILE) as xls:
        try:
            ...
            print(f"Lỗi đọc file diode.xlsx: {e}")
            _sheets = {}
else:
    print("Không tìm thấy file diode.xlsx, chỉ sử dụng entry fields")
    _sheets = {}
...
class DiodeLogic:
    ...
    def _generate_pn_other(self, class_code, values):
        ...
        """
        id_code = values.get("Identification code (4 digits)", "").strip()
    ...
    def generate_description(self, class_code, display_values, code_values, group="Other"):
        ...
        if group == "ABM":
            ...
            # desc. format = DIODE {tran_type} {Device No.} 
