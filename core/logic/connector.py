...
CONNECTOR_FILE = os.path.join(BASE_DIR, "core", "data", "connector.xlsx")

# Đọc file nếu tồn tại, nếu không thì _sheets rỗng
_sheets = {}
if os.path.exists(CONNECTOR_FILE):
    with pd.ExcelFile(CONNECTOR_FILE) as xls:
        try:
            ...
            print(f"Lỗi đọc file connector.xlsx: {e}")
            _sheets = {}
else:
    print("Không tìm thấy file connector.xlsx, chỉ sử dụng entry fields")
    _sheets = {}
...
class ConnectorLogic:
    ...
    PARAM_STRUCT = {
        "default": [
            ("No. of Contact Pins (3 chars)", "entry", None), 
    ...
    def _generate_pn_other(self, class_code, values):
        # Sinh part number theo quy tắc: P/N = 50{sub-classification}{No. of contact pins}{differ code}
        pin_no = values.get("No. of Contact Pins (3 chars)", "").strip()
        diff_code = values.get("Differ code (2 chars)", "").strip()
        
        # Đảm bảo pin number có 5 ký tự
        if len(pin_no) > 3:
            pin_no = pin_no[:3]
        return f"{class_code}{pin_no}{diff_code}R1F"
    ...
    def generate_description(self, class_code, display_values, code_values, group="Other"):
        ...
        if group == "ABM":
            ...
            """
            # desc. format = CONN {tran_type} {Device No.} 
            #tran_type = self._get_component_type_name(class_code)
            no_of_pin = display_values.get("No. of Contact Pins (3 chars)", "")
            diff = display_values.get("Differ code (2 chars)", "")
            
            desc_parts = ["CONN"]
            if no_of_pin:
                desc_parts.append(no_of_pin)
            if no_of_pin:
                desc_parts.append(no_of_pin)
            if no_of_pin:
                desc_parts.append(no_of_pin)
            ...
            """
            return f"Phải tự đặt thôi, quy tắc đặt desc cho connector vớ vẩn quá, code to tay vẫn lỗi"
    
    def generate_parameter(self, class_code, values, group="Other"):
        #Tạo description dạng text.
        # Đơn giản: liệt kê các trường đã nhập
        """
        desc_parts = ["Connector"]
        for key, val in values.items():
            if val:
                desc_parts.append(f"{key}: {val}")
        """
        return f"Vì không đặt desc, nên phần này thôi không làm, hihi"
