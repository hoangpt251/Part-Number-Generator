...
POWER_CONTROL_DEVICE_FILE = os.path.join(BASE_DIR, "core", "data", "power_control_device.xlsx")

# Đọc file nếu tồn tại, nếu không thì _sheets rỗng
_sheets = {}
if os.path.exists(POWER_CONTROL_DEVICE_FILE):
    with pd.ExcelFile(POWER_CONTROL_DEVICE_FILE) as xls:
        try:
            ...
            print(f"Lỗi đọc file power_control_device.xlsx: {e}")
            _sheets = {}
else:
    print("Không tìm thấy file power_control_device.xlsx, chỉ sử dụng entry fields")
    _sheets = {}

class Power_Control_DeviceLogic:
    ...
    def _generate_pn_other(self, class_code, values):
        """
            ...
            P/N = {class-code}{identification-code}{differ-code}
        """
        id_code = values.get("Identification code (4 chars)", "").strip()
        differ_code = values.get("Differ code (1 char)", "")
        
        # Đảm bảo pin number có 4 ký tự
        if len(id_code) > 4:
            id_code = id_code[:4]
        
        return f"{class_code}{id_code}{differ_code}R1F"
    ...
    def generate_description(self, class_code, display_values, code_values, group="Other"):
        ...
        if group == "ABM":
            ...
            # desc. format = POW {comp_type} {Device No.} 
    ...
    def generate_parameter(self, class_code, values, group="Other"):
        ...
        if group == "ABM":
            ...
            parts.append("CNN/CKR Power Device")
