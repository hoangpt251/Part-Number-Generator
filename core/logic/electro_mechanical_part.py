"""
    ...
    04/2026
    Tệp: electro_mechanical_part.py
"""
...
ELECTRO_MECHANICAL_PART_FILE = os.path.join(BASE_DIR, "core", "data", "electro_mechanical_part.xlsx")

# Đọc các sheet một lần khi import module
_sheets = {}
if os.path.exists(ELECTRO_MECHANICAL_PART_FILE):
    with pd.ExcelFile(ELECTRO_MECHANICAL_PART_FILE) as xls:
        try:
            ...
            print(f"Lỗi đọc file electro_mechanical_part.xlsx: {e}")
            _sheets = {}
else:
    print("Không tìm thấy file electro_mechanical_part.xlsx, chỉ sử dụng entry fields")
    _sheets = {}

class Electro_Mechanical_PartLogic:
    """
    ...
    PARAM_STRUCT = {
        ...
        # Sử dụng chung một loại định nghĩa, dùng default
    }
    ...
    def generate_description(self, class_code, display_values, code_values, group="Other"):
        if group == "ABM":
            ...
            return f"Chờ sang bản cập nhật sau bạn nhé :D"
        
    def generate_parameter(self, class_code, values, group="Other"):
        return f"Như dòng bên trên bạn à :D"