...
TRANSISTOR_FILE = os.path.join(BASE_DIR, "core", "data", "transistor.xlsx")

# Đọc file nếu tồn tại, nếu không thì _sheets rỗng
_sheets = {}
if os.path.exists(TRANSISTOR_FILE):
    with pd.ExcelFile(TRANSISTOR_FILE) as xls:
        try:
            ...
            print(f"Lỗi đọc file transistor.xlsx: {e}")
            _sheets = {}
else:
    print("Không tìm thấy file transistor.xlsx, chỉ sử dụng entry fields")
    _sheets = {}
...
class TransistorLogic:
    ...
    def _generate_pn_other(self, class_code, values):
        # Sinh part number theo quy tắc: P/N = {class-code}{identification-code}{differ-code}
    ...
    def generate_description(self, class_code, display_values, code_values, group="Other"):
        ...
        if group == "ABM":
            ...
            # desc. format = XSTR {tran_type} {Device No.} 
    ...
    def generate_parameter(self, class_code, values, group="Other"):
        ...
        if group == "ABM":
            ...
            parts.append("CNN/CKR Transistor")
