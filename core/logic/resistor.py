"""
    Chương trình tạo mã nội bộ và description
    Phát triển và kiểm thử: Phạm Trần Hoàng
    03/2026
    Tệp: resistor.py
"""

import os
import pandas as pd
from tkinter import messagebox

# Đường dẫn đến file dữ liệu của resistor
BASE_DIR = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
RESISTOR_FILE = os.path.join(BASE_DIR, "core", "data", "resistor.xlsx")

# Đọc các sheet một lần khi import module
_sheets = {}
if os.path.exists(RESISTOR_FILE):
    with pd.ExcelFile(RESISTOR_FILE) as xls:
        try:
            for sheet in xls.sheet_names:
                df = pd.read_excel(xls, sheet_name=sheet, dtype=str, header=0)
                df = df.fillna("")
                if df.shape[1] >= 2:
                    df = df.iloc[:, :2]
                    df.columns = ["display", "code"]
                    _sheets[sheet] = df
                    #print(f"Đã tải sheet: {sheet}")
        except Exception as e:
            print(f"Lỗi đọc file resistor.xlsx: {e}")
            _sheets = {}
else:
    print("Không tìm thấy file resistor.xlsx, chỉ sử dụng entry fields")
    _sheets = {}

class ResistorLogic:
    """ Xử lý tham số và sinh Part Number cho linh kiện Resistor.
        Hỗ trợ cả 3 group: ABM, CNN/CKR, Other
        Ánh xạ class-code -> cấu trúc tham số cho group Other
        Mỗi phần tử là tuple: (tên trường, loại, nguồn dữ liệu)
        Loại: 'entry' (textbox), 'combo' (combobox kèm sheet name)
    """   
    PARAM_STRUCT = {
        "10": [
            ("Watt", "combo", "Watt_6.3.1"),
            ("Resistance (2 chars)", "entry", None),
            ("Multiplier", "combo", "Multiplier"),
            ("Tolerance", "combo", "Tolerance"),
            ("Package", "combo", "Package_6.3.4")
        ],
        "11": [
            ("Watt", "combo", "Watt_6.3.1"),
            ("Resistance (3 chars)", "entry", None),
            ("Multiplier", "combo", "Multiplier"),
            ("Tolerance", "combo", "Tolerance"),
            ("Package", "combo", "Package_6.3.4")
        ],
        "16": [
            ("Watt", "combo", "Watt_6.5.7.1"),
            ("Resistance (3 chars)", "entry", None),
            ("Multiplier", "combo", "Multiplier"),
            ("Tolerance", "combo", "Tolerance"),
            ("Package", "combo", "Package_6.3.4")
        ],
        "12": [
            ("Watt", "combo", "Watt_6.3.1"),
            ("Resistance (2 chars)", "entry", None),
            ("Multiplier", "combo", "Multiplier"),
            ("Tolerance", "combo", "Tolerance"),
            ("Differ code (1 char)", "entry", None)
        ],
        "13": [
            ("Watt", "combo", "Watt_6.5.1"),
            ("Resistance (2 chars)", "entry", None),
            ("Multiplier", "combo", "Multiplier"),
            ("Tolerance", "combo", "Tolerance"),
            ("Differ code (1 char)", "entry", None)
        ],
        "140": [
            ("Resistance (2 chars)", "entry", None),
            ("Multiplier", "combo", "Multiplier"),
            ("Tolerance", "combo", "Tolerance"),
            ("Differ code (1 char)", "entry", None),
            ("Package", "combo", "Package_6.5.2")
        ],
        "141": [
            ("Resistance (2 chars)", "entry", None),
            ("Multiplier", "combo", "Multiplier"),
            ("Tolerance", "combo", "Tolerance"),
            ("Differ code (1 char)", "entry", None),
            ("Package", "combo", "Package_6.5.2")
        ],
        "143": [("Serial Number (5 chars)", "entry", None)],
        "150": [
            ("Resistance (2 chars)", "entry", None),
            ("Multiplier", "combo", "Multiplier"),
            ("Tolerance", "combo", "Tolerance"),
            ("Differ code (1 char)", "entry", None)
        ],
        "154": [
            ("Resistance (2 chars)", "entry", None),
            ("Multiplier", "combo", "Multiplier"),
            ("Tolerance", "combo", "Tolerance"),
            ("Differ code (1 char)", "entry", None)
        ],
        "152": [
            ("RMS Voltage (3 chars)", "entry", None),
            ("Pulse Energy (2 chars)", "entry", None),
            ("Rating Voltage (only Desc.)", "entry", None)
        ],
        "153": [("Serial Number (5 chars)", "entry", None)],
        "155": [("Serial Number (5 chars)", "entry", None)],
        "180": [
            ("Resistance (2 chars)", "entry", None),
            ("Multiplier", "combo", "Multiplier"),
            ("Tolerance", "combo", "Tolerance"),
            ("Differ code (1 char)", "entry", None)
        ],
        "181": [
            ("Resistance (2 chars)", "entry", None),
            ("Multiplier", "combo", "Multiplier"),
            ("Tolerance", "combo", "Tolerance"),
            ("Differ code (1 char)", "entry", None)
        ],
        "182": [
            ("Resistance (2 chars)", "entry", None),
            ("Multiplier", "combo", "Multiplier"),
            ("Tolerance", "combo", "Tolerance"),
            ("Differ code (1 char)", "entry", None)
        ],
    }
    
    # Định nghĩa extra fields cho ABM và CNN/CKR
    EXTRA_PARAM = {
        "ABM": [
            ("ABB Code", "entry", None),
            ("Suffix", "combo", "Suffix")   # Suffix sẽ được tạo trong __init__
        ],
        "CNN/CKR": [
                ("CKR/CNN Code (12 chars)", "entry", None),
                ("Revision (1 char)", "entry", None),
                ("RoHS Compliance", "combo", "ROHS"),
                ("MFG Identification (1 char)", "entry",None)
        ],
    }
     
    # Ánh xạ tên trường để lấy giá trị hiển thị từ combo
    DISPLAY_FIELDS = {
        "Watt": "Watt",
        "Multiplier": "Multiplier",
        "Tolerance": "Tolerance",
        "Package": "Package",
        "Temperature": "Temperature",
        "Voltage": "Voltage",
        "Suffix": "Suffix"
    }
    
    # Map multiplier để tính giá trị thực
    MULTIPLIER_MAP = {
        "10^0": 1,
        "10^1": 10,
        "10^2": 100,
        "10^3": 1000,
        "10^4": 10000,
        "10^5": 100000,
        "10^6": 1000000,
        "10^7": 10000000,
        "10^8": 100000000,
        "10^9": 1000000000,
        "10^-1": 0.1,
        "10^-2": 0.01,
        "10^-3": 0.001,
        "10^-4": 0.0001
    }
    
    def __init__(self):
        self.sheet_data = _sheets
        # Lưu mapping display -> code và code -> display cho các sheet
        self.display_to_code = {}
        self.code_to_display = {}
        for sheet_name, df in _sheets.items():
            self.display_to_code[sheet_name] = dict(zip(df["display"], df["code"]))
            self.code_to_display[sheet_name] = dict(zip(df["code"], df["display"]))
        
        # Tạo sheet Suffix
        if "Suffix" not in self.sheet_data:
            # Tạo DataFrame cho suffix
            suffix_data = {
                "display": ["RoHS", "Green"],
                "code": ["F", "G"]
            }
            self.sheet_data["Suffix"] = pd.DataFrame(suffix_data)
            self.display_to_code["Suffix"] = {"RoHS": "F", "Green": "G"}
            self.code_to_display["Suffix"] = {"F": "RoHS", "G": "Green"}
        
        # Tạo sheet ROHS
        if "ROHS" not in self.sheet_data:
            rohs_data ={
                "display": ["RoHS", "Non-RoHS"],
                "code": ["F", "Q"]
            }
            self.sheet_data["ROHS"] = pd.DataFrame(rohs_data)
            self.display_to_code["ROHS"] = {"RoHS": "F", "Non-RoHS": "Q"}
            self.code_to_display["ROHS"] = {"F": "RoHS", "Q": "Non-RoHS"}

    def get_parameter_fields(self, class_code, group="Other"):
        """ Trả về danh sách các trường cần nhập.
            - Default: lấy từ PARAM_STRUCT
            - ABM: lấy từ PARAM_STRUCT (nếu có) + thêm ABB Code và Suffix
            - CNN/CKR: lấy từ PARAM_STRUCT (nếu có) + thêm các trường đặc biệt
        """
        # Lấy base fields từ PARAM_STRUCT (nếu có)
        base_fields = self.PARAM_STRUCT.get(class_code, [])
        if group == "ABM":
            extra = self.EXTRA_PARAM["ABM"]
            fields = base_fields + extra
        elif group == "CNN/CKR":
            extra = self.EXTRA_PARAM["CNN/CKR"]
            fields = base_fields + extra
        else:
            fields = base_fields

        # Chuyển đổi fields thành dạng có display list và code map cho combo
        result = []
        for name, typ, source in fields:
            if typ == "combo":
                df = self.sheet_data.get(source, pd.DataFrame())
                if not df.empty:
                    display_list = df["display"].tolist()
                    code_map = dict(zip(df["display"], df["code"]))
                    result.append((name, "combo", display_list, code_map))
                else:
                    # Fallback: entry nếu không có sheet
                    result.append((name, "entry", None, None))
            else:
                result.append((name, "entry", None, None))
        return result

    def generate_part_number(self, class_code, values, group="Other"):
        """ Sinh part number dựa trên group.
            values: dict {tên trường: giá trị đã nhập/chọn}
        """
        if group == "ABM":
            return self._generate_pn_abm(class_code, values)
        elif group == "CNN/CKR":
            return self._generate_pn_ckr(class_code, values)
        else:
            return self._generate_pn_other(class_code, values)

    """ Tạo ra part no cho các nhóm không thuộc ABM, CKR/CNN """
    def _generate_pn_other(self, class_code, values):
        # Xác định format dựa trên class_code
        if class_code == "10":
            # 10{watt-code}{resistance value}{multiplier-code}{tolerance-code}"0-"{package-code}"R1F"
            watt = values.get("Watt", "")
            res_val = values.get("Resistance (2 chars)", "")
            mult = values.get("Multiplier", "")
            tol = values.get("Tolerance", "")
            pkg = values.get("Package", "")
            return f"10{watt}{res_val}{mult}{tol}0-{pkg}R1F"
        elif class_code in ("11", "16"):
            # {class-code}{watt-code}{resistance value}{multiplier-code}{tolerance-code}{package-code}"R1F"
            watt = values.get("Watt", "")
            res_val = values.get("Resistance (3 chars)", "")
            mult = values.get("Multiplier", "")
            tol = values.get("Tolerance", "")
            pkg = values.get("Package", "")
            return f"{class_code}{watt}{res_val}{mult}{tol}-{pkg}R1F"
        elif class_code in ("12", "13"):
            # {class-code}{watt-code}{resistance-value}{multiplier-code}{tolerance-code}{differ-code}
            watt = values.get("Watt", "")
            res_val = values.get("Resistance (2 chars)", "")
            mult = values.get("Multiplier", "")
            tol = values.get("Tolerance", "")
            diff = values.get("Differ code (1 char)", "")
            return f"{class_code}{watt}{res_val}{mult}{tol}{diff}R1F"
        elif class_code in ("140", "141"):
            # {class-code}{resistance-value}{multiplier-code}{tolerance-code}{differ-code}-{package-code}"R1F"
            res_val = values.get("Resistance (2 chars)", "")
            mult = values.get("Multiplier", "")
            tol = values.get("Tolerance", "")
            diff = values.get("Differ code (1 char)", "")
            pkg = values.get("Package", "")
            return f"{class_code}{res_val}{mult}{tol}{diff}-{pkg}R1F"
        elif class_code in ("143", "153", "155"):
            # {class-code}{serial-number}"R1F"
            sn_no = values.get("Serial Number (5 chars)", "")
            return f"{class_code}{sn_no}R1F"
        elif class_code in ("150", "154", "180", "181", "182"):
            # {class-code}{resistance-value}{multiplier-code}{tolerance-code}{differ-code}"R1F"
            res_val = values.get("Resistance (2 chars)", "")
            mult = values.get("Multiplier", "")
            tol = values.get("Tolerance", "")
            diff = values.get("Differ code (1 char)", "")
            return f"{class_code}{res_val}{mult}{tol}{diff}R1F"
        elif class_code == "152":
            # 152{rms-voltage}{pulse energy}"R1F"
            rms_voltage = values.get("RMS Voltage (3 chars)", "")
            energy = values.get("Pulse Energy (2 chars)", "")
            return f"152{rms_voltage}{energy}R1F"
        else:
            # Mặc định: ghép tất cả giá trị theo thứ tự fields
            return class_code + "".join(values.values())

    """ Tạo ra part no cho ABM """
    def _generate_pn_abm(self, class_code, values):
        class_prefix = class_code[:2] if len(class_code) >= 2 else class_code # Lấy 2 ký tự đầu của class-code
        abb_code = values.get("ABB Code", "").strip() # Xử lý ABB code
        
        # Điều kiện ràng buộc
        if len(abb_code) < 14: # Sử dụng trực tiếp    
            abb_processed_code = abb_code
        elif len(abb_code) == 14 or len(abb_code) == 15:
            if abb_code.startswith('3A'):    
                abb_processed_code = abb_code[2:] # Loại bỏ 3A và lấy phần phía sau
            else:
                # Cảnh báo nhưng vẫn dùng
                messagebox.showwarning("Cảnh báo", "Kiểm tra lại ABB sub-code đã nhập, độ dài chuỗi nhỏ hơn 14 ký tự.")
                abb_processed_code = abb_code
        else: # Nếu len(ABB_code) > 15, vẫn dùng nhưng cảnh báo
            messagebox.showwarning("Cảnh báo", "Kiểm tra lại ABB sub-code đã nhập, độ dài chuỗi nhỏ hơn 14 ký tự.")
            abb_processed_code = abb_code
        
        suffix = values.get("Suffix", "").strip() # Lấy suffix (có thể là code từ combo) 
        pn = f"{class_prefix}{abb_processed_code}W1{suffix}" # Tạo PN
        return pn

    """ Tạo ra part no cho CKR/CNN """
    def _generate_pn_ckr(self, class_code, values):
        class_prefix = class_code[:2] if len(class_code) >= 2 else class_code
        
        revision = values.get("Revision (1 char)", "").strip() # Lấy revision
        f_part = values.get("RoHS Compliance", "").strip() # “F” for RoHS; “Q” for Non-RoHS
        id_code = values.get("MFG Identification (1 char)", "").strip() # Manufacturer Identification
        ident_code = values.get("CKR/CNN Code (12 chars)", "").strip() 
        # Đảm bảo ident_code có 12 ký tự
        if len(ident_code) > 12:
            ident_code = ident_code[:12]
        elif len(ident_code) < 12:
            ident_code = ident_code.ljust(12, '0')           
        pn = f"{class_prefix}{ident_code}R{revision}{f_part}{id_code}"
        return pn

    """Lấy giá trị hiển thị từ code (dùng cho description)"""
    def _get_display_value(self, field_name, code_value):
        sheet_name = None
        for key in self.DISPLAY_FIELDS:
            if self.DISPLAY_FIELDS.get(key) == field_name or key == field_name:
                for class_code, fields in self.PARAM_STRUCT.items():
                    for f in fields:
                        if f[0] == field_name and f[1] == "combo":
                            sheet_name = f[2]
                            break
                    if sheet_name:
                        break
                break
        
        if sheet_name and sheet_name in self.code_to_display:
            return self.code_to_display[sheet_name].get(code_value, code_value)
        return code_value

    def _format_resistance_value(self, resistance_value, multiplier_code):
        """ Kết hợp resistance value và multiplier để ra giá trị thực với đơn vị R, K, M
            resistance_value: số nhập vào (string)
            multiplier_code: mã multiplier (string)
        """
        try:
            # Lấy giá trị hiển thị của multiplier
            multiplier_display = None
            if "Multiplier" in self.code_to_display:
                multiplier_display = self.code_to_display["Multiplier"].get(multiplier_code, multiplier_code)

            mult_value = self.MULTIPLIER_MAP.get(multiplier_display, 1) # Chuyển multiplier display thành số

            # Tính giá trị thực
            res_num = float(resistance_value) if resistance_value else 0
            real_value = res_num * mult_value 

            # Định dạng với đơn vị phù hợp
            if real_value >= 1_000_000_000:
                value = real_value / 1_000_000_000
                if value.is_integer(): # Giga-Ohm
                    return f"{int(value)}G" # Làm tròn
                else:
                    return f"{value:.1f}G".rstrip('0').rstrip('.')
            elif real_value >= 1_000_000:
                value = real_value / 1_000_000 # Mega-Ohm
                if value.is_integer(): # Làm tròn
                    return f"{int(value)}M"
                else:
                    return f"{value:.1f}M".rstrip('0').rstrip('.')
            elif real_value >= 1_000:
                value = real_value / 1_000 # Kilo-Ohm
                if value.is_integer():
                    return f"{int(value)}K"
                else:
                    return f"{value:.1f}K".rstrip('0').rstrip('.')
            elif real_value < 1:
                return f"{real_value}R" # Nếu giá trị nhỏ hơn 1, thì giữ nguyên
            else:
                if real_value.is_integer():
                    return f"{int(real_value)}R" # Ohm
                else:
                    return f"{real_value:.1f}R".rstrip('0').rstrip('.')
        except (ValueError, TypeError):
            return f"{resistance_value}{multiplier_code}" # Nếu lỗi thì trả về giá trị gốc

    def generate_description(self, class_code, display_values, code_values, group="Other"):
        """ Tạo description dạng text.
            Quy tắc: Nếu group là ABM thì sẽ yêu cầu lấy description trong bom của ABM
            Nếu group ko phải là ABM thì quy tắc đặt tên sẽ như nhau.
        """
        if group == "ABM":
            return f"Hãy copy lại description theo BOM của ABB"
        else:
            # Xử lý riêng cho resistance value và multiplier để có giá trị thực
            if "Resistance (2 chars)" in display_values and "Multiplier" in display_values:
                resistance_raw = display_values["Resistance (2 chars)"]
                multiplier_code = display_values.get("Multiplier", "")
                display_values["Resistance formatted"] = self._format_resistance_value(resistance_raw, multiplier_code)
            elif "Resistance (3 chars)" in display_values and "Multiplier" in display_values:
                resistance_raw = display_values["Resistance (3 chars)"]
                multiplier_code = display_values.get("Multiplier", "")
                display_values["Resistance formatted"] = self._format_resistance_value(resistance_raw, multiplier_code)

            # Tạo description theo từng class-code
            if class_code in ("10", "11"):
                # desc. format = RES bbbW ccc eee% fff ggg
                watt = display_values.get("Watt", "")
                resistance = display_values.get("Resistance formatted", "")
                tolerance = display_values.get("Tolerance", "")
                package = display_values.get("Package", "")
                other = display_values.get("Differ code", "") or display_values.get("Serial number", "") or ""
                
                desc_parts = ["RES"]
                if watt:
                    desc_parts.append(f"{watt}W")
                if resistance:
                    desc_parts.append(resistance)
                if tolerance:
                    desc_parts.append(tolerance)
                if package:
                    desc_parts.append(package)
                if other:
                    desc_parts.append(other)
                desc_parts.append("ROHS")
                return " ".join(desc_parts)
                
            elif class_code in ("12", "13", "140", "141", "143", "150", "153", "154", "155", "16", "180", "181", "182"):
                # desc. format = RES aaa bbbW ccc eee% fff ggg
                # Với các class-code này, aaa là component type (có thể lấy từ classification)
                aaa = self._get_component_type_name(class_code)
                watt = display_values.get("Watt", "")
                resistance = display_values.get("Resistance formatted", "")
                tolerance = display_values.get("Tolerance", "")
                package = display_values.get("Package", "")
                other = display_values.get("Differ code", "") or display_values.get("Serial number", "") or ""
                
                desc_parts = ["RES"]
                if aaa:
                    desc_parts.append(aaa)
                if watt:
                    desc_parts.append(f"{watt}W")
                if resistance:
                    desc_parts.append(resistance)
                if tolerance:
                    desc_parts.append(tolerance)
                if package:
                    desc_parts.append(package)
                if other:
                    desc_parts.append(other)
                desc_parts.append("ROHS")
                return " ".join(desc_parts)
                
            elif class_code == "152":
                # desc. format = RES aaa hhh eee% fff ggg
                aaa = self._get_component_type_name(class_code)
                voltage = display_values.get("RMS voltage applied", "")
                energy = display_values.get("Pulse energy", "")
                tolerance = display_values.get("Tolerance", "")
                package = display_values.get("Package", "")
                
                desc_parts = ["RES"]
                if aaa:
                    desc_parts.append(aaa)
                if voltage and energy:
                    desc_parts.append(f"{voltage}V/{energy}J")
                elif voltage:
                    desc_parts.append(f"{voltage}V")
                if tolerance:
                    desc_parts.append(tolerance)
                if package:
                    desc_parts.append(package)
                desc_parts.append("ROHS")
                return " ".join(desc_parts)
                
            else:
                # Mặc định: liệt kê các giá trị đã nhập
                desc_parts = ["Resistor"]
                for key, val in display_values.items():
                    if val and key not in ["Resistance formatted"]:
                        desc_parts.append(f"{key}: {val}")
                return ", ".join(desc_parts)
    
    def _get_component_type_name(self, class_code):
        type_map = {
            "12": "CC",
            "13": "WW",
            "140": "NTB",
            "141": "NTI",
            "150": "THM",
            "152": "VRS",
            "153": "PTR",
            "154": "FP",
            "16": "MOF",
            "180": "TRM",
            "181": "TRS",
            "182": "POT"
        }
        return type_map.get(class_code, "")

    # Hiển thi ra các parameter đã nhập
    def generate_parameter(self, class_code, values, group="Other"):
        parts = []
        # Xử lý theo group
        if group == "ABM":
            parts.append("ABM Resistor")
            abb = values.get("ABB Code", "")
            suffix = values.get("Suffix", "")
            suffix_display = self._get_display_value("Suffix", suffix) if suffix else suffix
            if abb:
                parts.append(f"ABB Code: {abb}")
            if suffix_display:
                parts.append(f"Suffix: {suffix_display}")        
        
        elif group == "CNN/CKR":
            parts.append("CNN/CKR Resistor")
            revision = values.get("Revision (1 char)", "")
            ident = values.get("CKR/CNN Code (12 chars)", "")
            if ident:
                parts.append(f"CNN/CKR Code: {ident}")
            if revision:
                parts.append(f"Revision: {revision}")
                
        else:  # group Other
            parts.append("Resistor")
            # Lấy giá trị hiển thị cho các combo field
            display_values = {}
            for key, val in values.items():
                if key in self.DISPLAY_FIELDS:
                    display_values[key] = self._get_display_value(key, val)
                else:
                    display_values[key] = val
            
            # Xử lý resistance value và multiplier
            if "Resistance (2 chars)" in display_values and "Multiplier" in values:
                resistance_raw = display_values.get("Resistance (2 chars)", "")
                multiplier_code = values.get("Multiplier", "")
                if resistance_raw and multiplier_code:
                    resistance_formatted = self._format_resistance_value(resistance_raw, multiplier_code)
                    parts.append(f"Resistance: {resistance_formatted}")
                    # Loại bỏ các key đã xử lý để tránh lặp
                    display_values.pop("Resistance (2 chars)", None)
                    # Multiplier không được thêm trực tiếp
            elif "Resistance (3 chars)" in display_values and "Multiplier" in values:
                resistance_raw = display_values.get("Resistance (3 chars)", "")
                multiplier_code = values.get("Multiplier", "")
                if resistance_raw and multiplier_code:
                    resistance_formatted = self._format_resistance_value(resistance_raw, multiplier_code)
                    parts.append(f"Resistance: {resistance_formatted}")
                    # Loại bỏ các key đã xử lý để tránh lặp
                    display_values.pop("Resistance (3 chars)", None)
                    # Multiplier không được thêm trực tiếp
            else:
                # Nếu không có multiplier, vẫn hiển thị resistance value
                if "Resistance (2 chars)" in display_values and display_values["Resistance (2 chars)"]:
                    parts.append(f"Resistance: {display_values['Resistance (2 chars)']} ohm")
                    display_values.pop("Resistance (2 chars)", None)
                elif "Resistance (3 chars)" in display_values and display_values["Resistance (3 chars)"]:
                    parts.append(f"Resistance: {display_values['Resistance (3 chars)']} ohm")
                    display_values.pop("Resistance (3 chars)", None)

            # Thêm các tham số còn lại
            for key, val in display_values.items():
                if val:
                    # Chuyển tên key thành dạng dễ đọc
                    display_key = key
                    if key == "Watt":
                        display_key = "Power"
                    elif key == "Tolerance":
                        display_key = "Tolerance"
                    elif key == "Package":
                        display_key = "Package"
                    elif key == "Temperature":
                        display_key = "Temp"
                    elif key == "Differ code":
                        display_key = "Differ"
                    elif key == "Serial number":
                        display_key = "Serial"
                    elif key == "RMS voltage applied":
                        display_key = "RMS Voltage"
                    elif key == "Pulse energy":
                        display_key = "Pulse Energy"
                    parts.append(f"{display_key}: {val}")
        
        return ", ".join(parts)